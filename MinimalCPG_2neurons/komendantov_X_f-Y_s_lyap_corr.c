...
// Case collected here: asymmetrical circuit with fast and slow current, from the study of X with values 1.0, 0.4 (first case reflected on the chapter).
// Furthermore, the currents are saved so that their shape can be investigated.
...
int main (int argc, char ** argv)
{
  ...
  FILE * salida, * spikes_X, * spikes_Y, * s_lyapunov, * s_corrientes_X, * s_corrientes_Y, * s_corrientes_con;

  char nombrearchivoV[80], nombrearchivoX [80], nombrearchivoY[80], nombrearchivoLyap[80], nombrearchivoCorr_X[80], nombrearchivoCorr_Y[80], nombrearchivoCorr_con[80];
  ...
  else
  {
    ...
    for(time = 0.0; time < time_max; time += inct)
    {
			...
			if((time > 1000) && conLyapunov) compone(vector, variables, eal);
			...
			if (punto==5)	
			{
  			...
  			if((time > 1000) && conLyapunov)
  			{
    			fprintf(s_lyapunov, "%lf ", time);
   				for(i = 0; i < DIM+1; i++) fprintf(s_lyapunov, "%0.5lg ", exponentes[i] / time);
    			fprintf(s_lyapunov, "\n");
  			}
			}
			...
			{
  			if (((variables_act[Vm_X] - variables_ant[Vm_X]) > 0) && ((variables_sig[Vm_X] - variables_act[Vm_X]) < 0))
  			{
    			if (contX == 0)
    			{
      			fprintf(spikes_X, "%d\n", 1);
			...
			{
  			if (((variables_act[Vm_Y] - variables_ant[Vm_Y]) > 0) && ((variables_sig[Vm_Y] - variables_act[Vm_Y]) < 0))
  			{
    			if (contY == 0)
    			{
      			fprintf(spikes_Y, "%d\n", 1);
...
void funcion(double tiempo, double * x,double * fvec)
{
	...
	fvec[Vm_X] = -(INaTTX_X + IKTEA_X + IK_X + INa_X + INaV_X + IB_X + ICa_X + ICa_Ca_X + I_s_X)/ Cm; 
	...
	fvec[ms_YX] = ((K1_X*(1.0-x[ms_YX]))/(1.0+exp(s_s*(V_s-x[Vm_Y])))) - K2_X*x[ms_YX];

	fvec[Vm_Y] = - (INaTTX_Y + IKTEA_Y + IK_Y + INa_Y + INaV_Y + IB_Y + ICa_Y + ICa_Ca_Y + I_f_Y) / Cm; 
