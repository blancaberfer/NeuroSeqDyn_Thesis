...
int main(int argc, char **argv) {
    ...
    int stopLP = 0, contLP = 0, stopPY = 0, contPY = 0, stopAB = 0, contAB = 0, stopPD1 = 0,
        contPD1 = 0, stopPD2 = 0, contPD2 = 0;
    ...
    char nombrearchivoV[80], nombrearchivoAB[80], nombrearchivoLP[80], nombrearchivoPD1[80],
        nombrearchivoPD2[80], nombrearchivoPY[80];
    ...
    for (g_PD1PD2 = g_PD1PD2_ini; g_PD1PD2 <= g_PD1PD2_max; g_PD1PD2 += g_Ds) {
        g_PD2PD1 = g_PD1PD2; // ** In this configuration, the current is symmetrical between all
                             // neurons in the pacemaker group. **
        g_ABPD1 = g_PD1PD2;
        g_PD1AB = g_PD1PD2;
        g_ABPD2 = g_PD1PD2;
        g_PD2AB = g_PD1PD2;
        ...
        sprintf(nombrearchivoV, "V_g_%.4f_g_%.4f", g_PD1PD2,
                g_PD2PD1); // File for saving potential values
        sprintf(nombrearchivoLP, "S_LP_g_%.4f_g_%.4f", g_PD1PD2,
                g_PD2PD1); // File for storing LP spikes
        sprintf(nombrearchivoPY, "S_PY_g_%.4f_g_%.4f", g_PD1PD2,
                g_PD2PD1); // File for storing PY spikes
        sprintf(nombrearchivoAB, "S_AB_g_%.4f_g_%.4f", g_PD1PD2,
                g_PD2PD1); // File for storing AB spikes
        sprintf(nombrearchivoPD1, "S_PD1_g_%.4f_g_%.4f", g_PD1PD2,
                g_PD2PD1); // File for storing PD1 spikes
        sprintf(nombrearchivoPD2, "S_PD2_g_%.4f_g_%.4f", g_PD1PD2,
                g_PD2PD1); // File for storing PD2 spikes

        if ((salida = fopen(nombrearchivoV, "r"))) { // ** We ensure the simulation hasn't been done
                                                     // before for that conductance value.  **
            fclose(salida);
            ...
            for (time = 0.0; time < time_max; time += inct) {
                ...
                if (punto == 5) {
                    fprintf(salida, "%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n", time, variables[Vm_LP],
                            variables[Vm_PY], variables[Vm_AB], variables[Vm_PD1],
                            variables[Vm_PD2]);
                    punto = 0;
                }
                ...
                if (variables_act[Vm_LP] > 0) {
                    if (((variables_act[Vm_LP] - variables_ant[Vm_LP]) > 0) &&
                        ((variables_sig[Vm_LP] - variables_act[Vm_LP]) < 0)) {
                        if (contLP == 0) {
                            fprintf(spikes_LP, "%d\n", 1);
                            contLP++;
                        }
                        if (stopLP == 0) {
                            fprintf(spikes_LP, "\n%.3f\t%.3f\t%d\t%d\n", time, variables_act[Vm_LP],
                                    1, 0);
                            stopLP = 1;
                        } else {
                            fprintf(spikes_LP, "\n%.3f\t%.3f\t%d\t%d\n", time, variables_act[Vm_LP],
                ...
                if (variables_act[Vm_PY] > 0) {
                    if (((variables_act[Vm_PY] - variables_ant[Vm_PY]) > 0) &&
                        ((variables_sig[Vm_PY] - variables_act[Vm_PY]) < 0)) {
                        if (contPY == 0) {
                            fprintf(spikes_PY, "%d\n", 1);
                            contPY++;
                        }
                        if (stopPY == 0) {
                            fprintf(spikes_PY, "\n%.3f\t%.3f\t%d\t%d\n", time, variables_act[Vm_PY],
                                    1, 0);
                            stopPY = 1;
                        } else {
                            fprintf(spikes_PY, "\n%.3f\t%.3f\t%d\t%d\n", time, variables_act[Vm_PY],
                ...
                if (variables_act[Vm_AB] > 0) {
                    if (((variables_act[Vm_AB] - variables_ant[Vm_AB]) > 0) &&
                        ((variables_sig[Vm_AB] - variables_act[Vm_AB]) < 0)) {
                        if (contAB == 0) {
                            fprintf(spikes_AB, "%d\n", 1);
                            contAB++;
                        }
                        if (stopAB == 0) {
                            fprintf(spikes_AB, "\n%.3f\t%.3f\t%d\t%d\n", time, variables_act[Vm_AB],
                                    1, 0);
                            stopAB = 1;
                        } else {
                            fprintf(spikes_AB, "\n%.3f\t%.3f\t%d\t%d\n", time, variables_act[Vm_AB],
                ...
                if (variables_act[Vm_PD1] > 0) {
                    if (((variables_act[Vm_PD1] - variables_ant[Vm_PD1]) > 0) &&
                        ((variables_sig[Vm_PD1] - variables_act[Vm_PD1]) < 0)) {
                        if (contPD1 == 0) {
                        ...
                        if (stopPD1 == 0) {
                            fprintf(spikes_PD1, "\n%.3f\t%.3f\t%d\t%d\n", time,
                                    variables_act[Vm_PD1], 1, 0);
                            stopPD1 = 1;
                        } else {
                            fprintf(spikes_PD1, "\n%.3f\t%.3f\t%d\t%d\n", time,
                                    variables_act[Vm_PD1], 0, 0);
                        }
                    }
                }
                ...
                if (variables_act[Vm_PD2] > 0) {
                    if (((variables_act[Vm_PD2] - variables_ant[Vm_PD2]) > 0) &&
                        ((variables_sig[Vm_PD2] - variables_act[Vm_PD2]) < 0)) {
                        if (contPD2 == 0) {
                        ...
                        if (stopPD2 == 0) {
                            fprintf(spikes_PD2, "\n%.3f\t%.3f\t%d\t%d\n", time,
                                    variables_act[Vm_PD2], 1, 0);
                            stopPD2 = 1;
                        } else {
                            fprintf(spikes_PD2, "\n%.3f\t%.3f\t%d\t%d\n", time,
                                    variables_act[Vm_PD2], 0, 0);
                        }
                    }
                }
...
void funcion(double tiempo, double *x, double *fvec) {
    ...
    INaTTX_LP =
        gNaTTX_cm * x[mNaTTX_LP] * x[mNaTTX_LP] * x[mNaTTX_LP] * x[hNaTTX_LP] * (x[Vm_LP] - VNa);
    IKTEA_LP = gKTEA_cm * x[nKTEA_LP] * x[nKTEA_LP] * x[nKTEA_LP] * x[nKTEA_LP] * (x[Vm_LP] - VK);
    ICa_LP = gCa_cm * x[mCa_LP] * x[mCa_LP] * (x[Vm_LP] - VCa);
    ICa_Ca_LP = gCa_Ca_cm * (x[Vm_LP] - VCa) /
                ((1.0 + exp(-0.06 * (x[Vm_LP] + 45.0))) * (1.0 + exp(kBeta * (x[Ca_LP] - beta))));
    I_f_LP = (g_PYLP_f * (x[Vm_LP] - E_syn)) / (1.0 + exp(s_f * (V_f - x[Vm_PY]))) +
             (g_ABLP_f * (x[Vm_LP] - E_syn)) / (1.0 + exp(s_f * (V_f - x[Vm_AB])));
    I_s_LP =
        g_PD1LP_s * x[ms_PD1LP] * (x[Vm_LP] - E_syn) + g_PD2LP_s * x[ms_PD2LP] * (x[Vm_LP] - E_syn);
    ...
    INaTTX_PY =
        gNaTTX_cm * x[mNaTTX_PY] * x[mNaTTX_PY] * x[mNaTTX_PY] * x[hNaTTX_PY] * (x[Vm_PY] - VNa);
    IKTEA_PY = gKTEA_cm * x[nKTEA_PY] * x[nKTEA_PY] * x[nKTEA_PY] * x[nKTEA_PY] * (x[Vm_PY] - VK);
    ICa_PY = gCa_cm * x[mCa_PY] * x[mCa_PY] * (x[Vm_PY] - VCa);
    ICa_Ca_PY = gCa_Ca_cm * (x[Vm_PY] - VCa) /
                ((1.0 + exp(-0.06 * (x[Vm_PY] + 45.0))) * (1.0 + exp(kBeta * (x[Ca_PY] - beta))));
    I_f_PY = (g_LPPY_f * (x[Vm_PY] - E_syn)) / (1.0 + exp(s_f * (V_f - x[Vm_LP]))) +
             (g_ABPY_f * (x[Vm_PY] - E_syn)) / (1.0 + exp(s_f * (V_f - x[Vm_AB])));
    I_s_PY =
        g_PD1PY_s * x[ms_PD1PY] * (x[Vm_PY] - E_syn) + g_PD2PY_s * x[ms_PD2PY] * (x[Vm_PY] - E_syn);
    ...
    INaTTX_AB =
        gNaTTX_r * x[mNaTTX_AB] * x[mNaTTX_AB] * x[mNaTTX_AB] * x[hNaTTX_AB] * (x[Vm_AB] - VNa);
    IKTEA_AB = gKTEA_r * x[nKTEA_AB] * x[nKTEA_AB] * x[nKTEA_AB] * x[nKTEA_AB] * (x[Vm_AB] - VK);
    ICa_AB = gCa_r * x[mCa_AB] * x[mCa_AB] * (x[Vm_AB] - VCa);
    ICa_Ca_AB = gCa_Ca_r * (x[Vm_AB] - VCa) /
                ((1.0 + exp(-0.06 * (x[Vm_AB] + 45.0))) * (1.0 + exp(kBeta * (x[Ca_AB] - beta))));
    ...
    INaTTX_PD1 = gNaTTX_cm * x[mNaTTX_PD1] * x[mNaTTX_PD1] * x[mNaTTX_PD1] * x[hNaTTX_PD1] *
                 (x[Vm_PD1] - VNa);
    IKTEA_PD1 =
        gKTEA_cm * x[nKTEA_PD1] * x[nKTEA_PD1] * x[nKTEA_PD1] * x[nKTEA_PD1] * (x[Vm_PD1] - VK);
    ICa_PD1 = gCa_cm * x[mCa_PD1] * x[mCa_PD1] * (x[Vm_PD1] - VCa);
    ICa_Ca_PD1 =
        gCa_Ca_cm * (x[Vm_PD1] - VCa) /
        ((1.0 + exp(-0.06 * (x[Vm_PD1] + 45.0))) * (1.0 + exp(kBeta * (x[Ca_PD1] - beta))));
    ...
    INaTTX_PD2 = gNaTTX_cbm * x[mNaTTX_PD2] * x[mNaTTX_PD2] * x[mNaTTX_PD2] * x[hNaTTX_PD2] *
                 (x[Vm_PD2] - VNa);
    IKTEA_PD2 =
        gKTEA_cbm * x[nKTEA_PD2] * x[nKTEA_PD2] * x[nKTEA_PD2] * x[nKTEA_PD2] * (x[Vm_PD2] - VK);
    ICa_PD2 = gCa_cbm * x[mCa_PD2] * x[mCa_PD2] * (x[Vm_PD2] - VCa);
    ICa_Ca_PD2 =
        gCa_Ca_cbm * (x[Vm_PD2] - VCa) /
        ((1.0 + exp(-0.06 * (x[Vm_PD2] + 45.0))) * (1.0 + exp(kBeta * (x[Ca_PD2] - beta))));
    I_e_PD2 = g_PD1PD2 * (x[Vm_PD2] - x[Vm_PD1]) + g_ABPD2 * (x[Vm_PD2] - x[Vm_AB]);
    I_f_PD2 = (g_LPPD2_f * (x[Vm_PD2] - E_syn)) / (1.0 + exp(s_f * (V_f - x[Vm_LP])));

    fvec[Vm_LP] = -(INaTTX_LP + IKTEA_LP + IK_LP + INa_LP + INaV_LP + IB_LP + ICa_LP + ICa_Ca_LP +
                    I_s_LP + I_f_LP) /
                  Cm;
    ...
    fvec[ms_PD1LP] = ((K1_LP * (1.0 - x[ms_PD1LP])) / (1.0 + exp(s_s * (V_s - x[Vm_PD1])))) -
                     K2_LP * x[ms_PD1LP];
    fvec[ms_PD2LP] = ((K1_LP * (1.0 - x[ms_PD2LP])) / (1.0 + exp(s_s * (V_s - x[Vm_PD2])))) -
                     K2_LP * x[ms_PD2LP];

    fvec[Vm_PY] = -(INaTTX_PY + IKTEA_PY + IK_PY + INa_PY + INaV_PY + IB_PY + ICa_PY + ICa_Ca_PY +
                    I_s_PY + I_f_PY) /
                  Cm;
    ...
    fvec[ms_PD1PY] = ((K1_PY * (1.0 - x[ms_PD1PY])) / (1.0 + exp(s_s * (V_s - x[Vm_PD1])))) -
                     K2_PY * x[ms_PD1PY];
    fvec[ms_PD2PY] = ((K1_PY * (1.0 - x[ms_PD2PY])) / (1.0 + exp(s_s * (V_s - x[Vm_PD2])))) -
                     K2_PY * x[ms_PD2PY];

    fvec[Vm_AB] =
        -(INaTTX_AB + IKTEA_AB + IK_AB + INa_AB + INaV_AB + IB_AB + ICa_AB + ICa_Ca_AB + I_e_AB) /
        Cm;
    ...
    fvec[Vm_PD1] = -(INaTTX_PD1 + IKTEA_PD1 + IK_PD1 + INa_PD1 + INaV_PD1 + IB_PD1 + ICa_PD1 +
                     ICa_Ca_PD1 + I_f_PD1 + I_e_PD1) /
                   Cm;
    ...
    fvec[Vm_PD2] = -(INaTTX_PD2 + IKTEA_PD2 + IK_PD2 + INa_PD2 + INaV_PD2 + IB_PD2 + ICa_PD2 +
                     ICa_Ca_PD2 + I_f_PD2 + I_e_PD2) /
                   Cm;
