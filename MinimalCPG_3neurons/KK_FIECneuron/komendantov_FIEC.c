...
//////////////////////////////////////////////////////////
//														//
//			CPG (FIEC configuration, KK model)		 	//
//														//
//////////////////////////////////////////////////////////

// CPG X-Y and Z, electrically coupled to Y : FIEC configuration
...
static double g_ZY = 0.0; // Electrical conductance variable from Z to Y
static double g_YZ = 0.0; // Electrical conductance variable from Y to Z
...
int main(int argc, char **argv) {
    ...
    double variables[DIM][DIM], variables_pre[DIM], variables_now[DIM], variables_aft[DIM];
    ...
    for (g_ZY = 0.0000; g_ZY <= g_max; g_ZY = g_ZY + d_g) {
        g_YZ = g_ZY; // ** Symmetric gap junction

        set_conditions(variables, variables_pre);

        sprintf(namefileV, "V_gZY_%.4f", g_ZY);   // File for saving potential values
        sprintf(namefileX, "S_X_gZY_%.4f", g_ZY); // File for storing X spikes
        sprintf(namefileY, "S_Y_gZY_%.4f", g_ZY); // File for storing Y spikes
        sprintf(namefileZ, "S_Z_gZY_%.4f", g_ZY); // File for storing Z spikes
        ...
        if (salida == NULL) {
            ...
            for (time = 0.0; time < time_max; time += inct) {
                ...
                if (point == 5) {
                    fprintf(salida, "%.4f\t%.4f\t%.4f\t", time, variables[Vm_X], variables[Vm_Y]);
                    fprintf(salida, "%.4f\t", variables[Vm_Z]);
                    fprintf(salida, "\n");
                    point = 0;
                }
                ...
                if (variables_now[Vm_X] > 0) {
                    if (((variables_now[Vm_X] - variables_pre[Vm_X]) > 0) &&
                        ((variables_aft[Vm_X] - variables_now[Vm_X]) < 0)) {
                        if (contX == 0) {
                            fprintf(spikes_X, "%d\n", 1);
                            contX++;
                        }
                        if (stopX == 0) {
                            fprintf(spikes_X, "\n%.4f\t%.4f\t%d\t%d\n", time, variables_now[Vm_X],
                                    1, 0);
                            stopX = 1;
                        } else {
                            fprintf(spikes_X, "\n%.4f\t%.4f\t%d\t%d\n", time, variables_now[Vm_X],
                ...
                if (variables_now[Vm_Y] > 0) {
                    if (((variables_now[Vm_Y] - variables_pre[Vm_Y]) > 0) &&
                        ((variables_aft[Vm_Y] - variables_now[Vm_Y]) < 0)) {
                        if (contY == 0) {
                            fprintf(spikes_Y, "%d\n", 1);
                            contY++;
                        }
                        if (stopY == 0) {
                            fprintf(spikes_Y, "\n%.4f\t%.4f\t%d\t%d\n", time, variables_now[Vm_Y],
                                    1, 0);
                            stopY = 1;
                        } else {
                            fprintf(spikes_Y, "\n%.4f\t%.4f\t%d\t%d\n", time, variables_now[Vm_Y],
                ...
                if (variables_now[Vm_Z] > 0) {
                    if (((variables_now[Vm_Z] - variables_pre[Vm_Z]) > 0) &&
                        ((variables_aft[Vm_Z] - variables_now[Vm_Z]) < 0)) {
                        if (contZ == 0) {
                            fprintf(spikes_Z, "%d\n", 1);
                            contZ++;
                        }
                        if (stopZ == 0) {
                            fprintf(spikes_Z, "\n%.4f\t%.4f\t%d\t%d\n", time, variables_now[Vm_Z],
                                    1, 0);
                            stopZ = 1;
                        } else {
                            fprintf(spikes_Z, "\n%.4f\t%.4f\t%d\t%d\n", time, variables_now[Vm_Z],
...
void kk_model(double tiempo, double *x, double *fvec) {
    ...
    I_s_YX = g_YX_s * x[ms_YX] * (x[Vm_X] - E_syn); // + g_YX_s * x[ms_YX] * (x[Vm_X] - E_syn);
    ...
    ICa_Ca_Z = gCa_Ca_cbm * (x[Vm_Z] - VCa) /
               ((1.0 + exp(-0.06 * (x[Vm_Z] + 45.0))) * (1.0 + exp(kBeta * (x[Ca_Z] - beta))));

    I_elec_ZY = g_ZY * (x[Vm_Y] - x[Vm_Z]);

    fvec[Vm_X] = -(INaTTX_X + IKTEA_X + IK_X + INa_X + INaV_X + IB_X + ICa_X + ICa_Ca_X + I_s_YX) /
                 Cm; // + I_elec_Z1X
    ...
    fvec[Vm_Y] = -(INaTTX_Y + IKTEA_Y + IK_Y + INa_Y + INaV_Y + IB_Y + ICa_Y + ICa_Ca_Y + I_f_XY +
                   I_elec_ZY) /
                 Cm; //
    ...
    fvec[ms_YX] =
        ((K1_s * (1.0 - x[ms_YX])) / (1.0 + exp(s_s * (V_s - x[Vm_Y])))) - K2_s * x[ms_YX];

    fvec[Vm_Z] =
        -(INaTTX_Z + IKTEA_Z + IK_Z + INa_Z + INaV_Z + IB_Z + ICa_Z + ICa_Ca_Z + I_elec_YZ) / Cm;
